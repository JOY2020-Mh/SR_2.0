...
class Solver():
    def __init__(self, model, cfg):
        ...
        self.refiner.load_state_dict(torch.load(cfg.pretrained_model, map_location = 'cpu'))
        print(self.refiner)
        ...
        self.train_data = TrainDatasetFromFolder(cfg.GT_dir, cfg.LR_dir, crop_size=cfg.patch_size, scale_factor=cfg.scale)
        self.train_loader = DataLoader(self.train_data, batch_size=cfg.batch_size, shuffle=True, num_workers= 0, pin_memory=True)
    ...
    def fit(self):
        ...
        while True:
            ...
            print("Epoch: [%2d] [%4d/%4d] loss: %8f" % ((epoch + 1), (iter + 1), len(self.train_loader), epoch_loss))
    ...
    # cfg = self.cfg
    # dataset = TestDataset(cfg.test_GT_dir, cfg.test_LR_dir, cfg.scale)
    # self.refiner.evaluate(dataset, scale = 4, num_step=0)

    def evaluate(self, test_dataset, scale=4, num_step=0):
