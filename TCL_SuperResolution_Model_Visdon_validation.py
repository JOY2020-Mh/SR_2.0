...
class TCL_SuperResolution(object):
    ...
    def load_dataset(self, dataset):
        ...
        if dataset == 'train':
            ...
            test_set = TestDatasetFromFolder(self.test_GT, self.test_dataset, crop_size=self.crop_size, scale_factor=self.scale_factor, is_gray=False, save_inImg=self.save_inImg, inYCbCr=self.inYCbCr)
            test_data_loader = DataLoader(test_set, batch_size=self.batch_size, shuffle=True, num_workers= self.num_threads, pin_memory=True)
    ...
    def decay_learning_rate(self, epoch):
        lr = self.lr * (0.5**(epoch//self.decay))
        return lr
        ...
        for epoch in range(self.num_epochs):
            ...
            for iter, data in enumerate(train_data_loader):
                ...
                if self.save_inImg == True:  #save the net input image
                    ...
                    imageio.imsave('predicted_Y/'+ str(iter) + 'predicted.png', saveinY[:, :, 0])
                    #scipy.misc.imsave('lrin.png', saveinY[:, :, 0]);

                    savetarY = (target_Y.numpy()[0, :, :, :].transpose(1, 2, 0)*255).astype(numpy.uint8)
                    imageio.imsave('target_Y/' + str(iter) + 'tarin3.png', savetarY[:, :, 0])
                ...
                #print('training set one batch loss', loss.data)
                step += 1
                
                #self.validation(epoch, val_data_loader)
            print(epoch, ' training set one epoch loss ', epoch_loss)
    ...
    def validation(self): #input as YCbC to be complete
        ...
        for iter, data in enumerate(val_data_loader):
            ...
            savein_target_Y = (target_Y.numpy()[0, :, :, :].transpose(1, 2, 0)*255).astype(numpy.uint8)
            saveinY = (recon_imgs.numpy()[0, :, :, :].transpose(1, 2, 0)*255).astype(numpy.uint8)
            #imageio.imsave('1116_validation_image/predicted/'+ str(iter) + 'predicted.png', saveinY[:, :, 0])
            #imageio.imsave('1116_validation_image/Target_Y/'+ str(iter) + 'target_Y.png', savein_target_Y[:, :, 0])
        ...
        print('the average validation dataset loss is', total_loss/len(val_data_loader))
