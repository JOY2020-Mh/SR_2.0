#### for training it is random crop and rotate and flip
#### in validation and testing all it has is centor crop
...
class TestDatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.save_inImg == True:
            imageio.imsave('1117_validation_image/img_HR'+str(index)+'.png', img_HR)
            imageio.imsave('1117_validation_image/img_LR'+str(index)+'.png', img_LR)
        #   img_HR.save('train_image/img_HR'+str(index)+'.png')
        #   img_LR.save('train_image/img_LR'+str(index)+'.png')
